# Legacy: NFL Draft MVP Implementation Plan

> **Note:** This plan described the original NFL Draft Retention & Role Tracker. The project has pivoted to **Hockey Event Ranking**. See [docs/MVP_SPEC.md](../MVP_SPEC.md) for the current spec. Kept for historical reference.

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal (historical):** Build a front-end-only static site that evaluated NFL draft success by team using snap share, games played, and retention. Team-centric view; data from version-controlled JSON; no backend.

**Architecture (historical):** Vite + React + TypeScript. Data in `public/data/draft-{year}.json`. Client-side role classification and metrics. Script updated JSON from nflverse.
