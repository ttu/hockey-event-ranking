...
2. Check CI/CD status first:
   ...
   ```bash
   gh pr checks {pr_number} --repo ttu/hockey-event-ranking
   ```
...
3. Fetch CodeRabbit review comments from the PR:
   ...
   gh api --paginate "repos/ttu/hockey-event-ranking/pulls/{pr_number}/comments" \
   ...
   gh api --paginate "repos/ttu/hockey-event-ranking/issues/{pr_number}/comments" \
...
5. Check SonarCloud issues:
   ...
   # https://sonarcloud.io/api/issues/search?componentKeys=ttu_hockey-event-ranking&pullRequest={pr_number}&resolved=false
   ```
   ...
   - Or visit web UI: `https://sonarcloud.io/project/issues?id=ttu_hockey-event-ranking&pullRequest={pr_number}`
...
7. Check human reviewer comments:
   ...
   gh api "repos/ttu/hockey-event-ranking/issues/{pr_number}/comments" \
...
1. Get comment IDs with context:
   ...
   ```bash
   gh api "repos/ttu/hockey-event-ranking/pulls/{pr_number}/comments" \
...
2. Reply to a comment explaining why it won't be fixed:
   ...
   ```bash
   gh api "repos/ttu/hockey-event-ranking/pulls/{pr_number}/comments/{comment_id}/replies" \
