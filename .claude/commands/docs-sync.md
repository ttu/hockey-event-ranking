...
- [ ] All TypeScript interfaces documented (`RosterPlayer`, `CompetitionSummary`, `TeamSummary`, `NationalPosition`, etc.)
- [ ] Player/score types if present (`Line`, `Role`, weights, etc.)
- [ ] Property names and types match exactly
- [ ] Optional vs required fields match (`?` in code vs "optional" in docs)
- [ ] JSON layout description matches actual `public/data/competitions/` structure

### 2. Data Schema Sync

**Source of Truth:** `public/data/competitions/` (or `scripts/update-data.ts` output schema)
**Documentation:** `docs/datamodel.md`

Verify these match:

- [ ] JSON file layout (competition id, teams, roster per team) matches docs
- [ ] RosterPlayer fields (playerId, name, nationalPosition, clubLine, clubRole, salary, score, etc.) match
- [ ] CompetitionSummary and TeamSummary structure match
...
- [ ] SPEC_CLARIFICATIONS.md rules match implementation (league/line/role weights, score formula)
